...
public:
    ...
    static void
    Current_State_init(general_state& general_state, operational_state& operational_state) {
        Current_State_packet =
            new HeapPacket(static_cast<uint16_t>(249), &general_state, &operational_state);
    }

    static void
    Reeds_init(reed1& reed1, reed2& reed2, reed3& reed3, reed4& reed4, bool& all_reeds) {
        Reeds_packet =
            new HeapPacket(static_cast<uint16_t>(251), &reed1, &reed2, &reed3, &reed4, &all_reeds);
    }
    ...
    static void Regulator_init(float& regulator_1_pressure, float& regulator_2_pressure) {
        Regulator_packet = new HeapPacket(
            static_cast<uint16_t>(252),
            &regulator_1_pressure,
            &regulator_2_pressure
        );
    }

    static void Pressure_init(
        float& pressure_high,
        float& pressure_regulator,
        float& pressure_brakes,
        float& pressure_capsule
    ) {
        Pressure_packet = new HeapPacket(
            static_cast<uint16_t>(253),
            &pressure_high,
            &pressure_regulator,
            &pressure_brakes,
            &pressure_capsule
    ...
    static void tapes_init(tape_enable_output& tape_enable_output, emergency_tape& emergency_tape) {
        tapes_packet =
            new HeapPacket(static_cast<uint16_t>(254), &tape_enable_output, &emergency_tape);
    }
