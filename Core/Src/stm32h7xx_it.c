    ...
    uint32_t addr_metadata, const void *data_metadata, size_t len_metadata)
{
    ...
    while(offset < len_hard_fault){
        ...
        if(HAL_FLASH_Program(FLASH_TYPEPROGRAM_FLASHWORD, addr_hard_fault + offset, block) != HAL_OK){
            __BKPT(0);
        }
        offset += 32;
    }
    ...
    while(offset < len_metadata){
        ...
        if(HAL_FLASH_Program(FLASH_TYPEPROGRAM_FLASHWORD, addr_metadata + offset, block) != HAL_OK){
            __BKPT(0);
        }
        offset += 32;
    }
